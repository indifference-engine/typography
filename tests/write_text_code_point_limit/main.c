...
int main(const int argc, const char *const *const argv)
{
  ...
  if (!write_text_code_point(402862483, &font_a, 0.4555927370f, 0.7303668130f, 0.0712067199f, 0.5033791132f, &state))
  {
  ...
  if (!write_text_code_point(505255796, &font_a, 0.0826451742f, 0.7329012169f, 0.3891597672f, 0.3621127009f, &state))
  {
    fprintf(stderr, "Unexpectedly failed to write sixth code point.\n");
    return 1;
  }

  if (!write_text_code_point(204668171, &font_a, 0.6479530738f, 0.2324556519f, 0.6618752571f, 0.2592586932f, &state))
  {
    fprintf(stderr, "Unexpectedly failed to write seventh code point.\n");
    return 1;
  }

  if (write_text_code_point(358868195, &font_b, 0.4104844575f, 0.1156985448f, 0.8581898866f, 0.7918596686f, &state))
  {
