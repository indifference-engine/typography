...
static void assert_int(const char *const description, const int expected,
                       const int actual) {
  if (expected != actual) {
...
static void assert_float(const char *const description, const float expected,
                         const float actual) {
  if (expected != actual) {
...
static void assert_pointer(const char *const description,
...
int main(const int argc, const char *const *const argv) {
  ...
  written_text written = start_text(
      7, written_text_fonts, written_glyph_indices, written_opacities,
      written_reds, written_greens, written_blues);

  const int glyph_widths_a[] = {23, 18, 19, 25};
  const uint32_t glyph_code_points_a[] = {505255796, 204668171, 358868195,
                                          402862483};
  const int glyph_row_offsets_a[] = {5, -7, 8, 2};

  const text_font font_a = {.code_point_spacing = 5,
                            .line_spacing = 3,
                            .line_height = 12,
  ...
  const text_font font_b = {.code_point_spacing = 41,
                            .line_spacing = 25,
                            .line_height = 17,
  ...
  write_text_code_point(204668171, &font_a, 0.1564262168f, 0.0437286089f,
                        0.2452098142f, 0.4578030974f, &written);
  write_text_code_point(505255796, &font_b, 0.3664873978f, 0.8843461135f,
                        0.0204608919f, 0.8177569126f, &written);
  write_text_code_point(204668171, &font_b, 0.5377511140f, 0.6091213575f,
                        0.3598658273f, 0.4994471018f, &written);
  write_text_code_point(37498374, &font_b, 0.1292127857f, 0.3058116156f,
                        0.5600688226f, 0.6124764171f, &written);
  write_text_code_point(505255796, &font_a, 0.0826451742f, 0.7329012169f,
                        0.3891597672f, 0.3621127009f, &written);
  write_text_code_point(204668171, &font_a, 0.6479530738f, 0.2324556519f,
                        0.6618752571f, 0.2592586932f, &written);
  ...
  place_text(written, ABOVE_TEXT_ANCHOR, RIGHT_OF_TEXT_ANCHOR, 1000, -2000,
             &placed);
  ...
  assert_int("placed glyph row a", 981, placed_glyph_rows[0]);
  assert_int("placed glyph row b", 992, placed_glyph_rows[1]);
  assert_int("placed glyph row c", 986, placed_glyph_rows[2]);
  assert_int("placed glyph row d", 981, placed_glyph_rows[3]);
  assert_int("placed glyph row e", 993, placed_glyph_rows[4]);
  assert_int("placed glyph row f", 981, placed_glyph_rows[5]);
  ...
  assert_int("placed glyph column d", -1760, placed_glyph_columns[3]);
  assert_int("placed glyph column e", -1682, placed_glyph_columns[4]);
  assert_int("placed glyph column f", -1654, placed_glyph_columns[5]);

  assert_float("placed glyph opacity a", 0.1564262168f,
               placed_glyph_opacities[0]);
  assert_float("placed glyph opacity b", 0.3664873978f,
               placed_glyph_opacities[1]);
  assert_float("placed glyph opacity c", 0.5377511140f,
               placed_glyph_opacities[2]);
  assert_float("placed glyph opacity d", 0.1292127857f,
               placed_glyph_opacities[3]);
  assert_float("placed glyph opacity e", 0.0826451742f,
               placed_glyph_opacities[4]);
  assert_float("placed glyph opacity f", 0.6479530738f,
               placed_glyph_opacities[5]);
